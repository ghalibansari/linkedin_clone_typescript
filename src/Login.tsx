...
function Login() {
    ...
    const loginToApp = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.preventDefault();

        if(!email) return alert('Enter email')
        else if(!password) return alert('Enter password')
    ...
    const register = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        else if(!email) return alert('Enter email')
        else if(!password) return alert('Enter password')

        auth.createUserWithEmailAndPassword(email, password)
        .then(userAuth => {
            userAuth.user?.updateProfile({displayName: name, photoURL: profilePic})
            .then(() => {
                dispatch(login({
                    email: userAuth.user?.email,
                    uid: userAuth.user?.uid,
